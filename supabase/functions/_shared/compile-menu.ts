...
// is noise, not a benign extra option. Per docs/specs/2026-09-15-code-owned-
// resolution.md §3 work item 1: `fries` and `burger` are supposed to drop as
// ambiguous (10 and 7 Vito's items respectively) — that's the gate working,
...
// One candidate pool + one uniqueness gate: propose every (term, itemId)
// pair `generate` produces off `source`, skip anything already in
// `excluded`, then keep only terms exactly one item id claims. Order of
// `source`/`out` doesn't affect which candidates survive (uniqueness is
// symmetric, not first-wins) — every caller's final result still goes
// through dedupeLexicon before being merged into an item's lexicon_terms,
// so output order is deterministic regardless.
function gateSurfaceFormCandidates(
  source: SurfaceFormCandidate[],
  excluded: Set<string>,
  generate: (term: string) => string[],
): SurfaceFormCandidate[] {
  const candidates: SurfaceFormCandidate[] = [];
  ...
  for (const { term, itemId } of source) {
    for (const candidate of generate(term)) propose(candidate, itemId);
  }
