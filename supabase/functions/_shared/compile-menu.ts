...
// "cheeseburger") and plural ("burger" -> "burgers"). Real 2026-09-14 bug:
// Vito's had "cheese burger" ($8.49) and "bacon cheeseburger" ($10.99) but
// no term for the bare word "cheeseburger" a customer actually types, so it
// fell through to whichever OTHER term happened to contain that substring
// and resolved to the wrong, dearer item on 11/17 live calls. Per the PO's
// ruling, this is compiler-generated data anticipating real customer
// phrasing, not a one-off fix for this one item — so it runs for every
// active item term, every menu, every shop.
//
// Uniqueness is the ONLY gate, never a heuristic tiebreak: a candidate that
// would resolve to more than one target across the whole shop lexicon
// (existing rule 1/2/3/6 terms, OR another candidate generated by this same
// pass) is dropped outright for every claimant — never kept for whichever
// item happened to generate it first. A term nobody could type unambiguously
// is noise, not a benign extra option.
// ============================================================
...
// Iterates compiledItems/lexicon_terms in their existing stable order
// (compiledItems is items.map(...); each item's terms are emitted in rule
// order), space-collapse before pluralization for a given source term, so
// output order — and therefore which single claimant survives a tie in
// `proposed` — is reproducible across runs on identical input.
function deriveLexiconSurfaceForms(compiledItems: CompiledItem[], categoryLexicon: LexiconTerm[]): LexiconTerm[] {
  // The shop's lexicon as it exists before this pass (every rule 1/2/6 item/
  // choice term, every rule 3 category term). A candidate matching any of
  // these already "exists as a term" (own item) or would create a
  // cross-target ambiguity (any other target) — either way, don't add it.
  const existing = new Set<string>();
  for (const c of compiledItems) for (const t of c.lexicon_terms) existing.add(t.term);
  for (const t of categoryLexicon) existing.add(t.term);

  const candidates: { term: string; itemId: string }[] = [];
  const claimants = new Map<string, Set<string>>(); // term -> distinct item ids proposing it

  const propose = (term: string, itemId: string) => {
    if (existing.has(term)) return;
    candidates.push({ term, itemId });
    const set = claimants.get(term) ?? new Set<string>();
    set.add(itemId);
    claimants.set(term, set);
  };

  for (const c of compiledItems) {
    for (const t of c.lexicon_terms) {
      if (t.target_type !== "item") continue;
      const collapsed = collapseSpaces(t.term);
      if (collapsed !== t.term) propose(collapsed, t.target_id);

      const plural = pluralizeSurfaceForm(t.term);
      if (plural && plural !== t.term) propose(plural, t.target_id);
    }
  }
  ...
  for (const cand of candidates) {
    if (emitted.has(cand.term)) continue;
    emitted.add(cand.term);
    if (claimants.get(cand.term)!.size > 1) continue; // ambiguous — drop for every claimant, no tiebreak
    out.push({ term: cand.term, target_type: "item", target_id: cand.itemId, provenance: "derived" });
  }
  return out;
}
...
export function compileMenu(
  ...
  const surfaceForms = deriveLexiconSurfaceForms(compiledItems, categoryLexicon);
