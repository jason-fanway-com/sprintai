...
function e1GuardWarnLine(userMessage: string, cartId: string, cartLength: number): string | null {
  ...
  if (isAdditive && !isExplicitRestart && cartLength > 0) {
    // This is the exact line from index.ts:1750 (post-fix). If a future edit
    // reintroduces a reference to an out-of-scope identifier here, this test
    // throws the same ReferenceError the live bot did.
