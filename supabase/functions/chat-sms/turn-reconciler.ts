...
// PO 2026-09-13 ($341.98 live incident, matrix run 3 of 5): a bare number in a
// turn ("yep, and two cokes") was parsed by parseExplicitQuantity from the WHOLE
// turn text and then handed to EVERY writeCartLine call in that turn — including
// the pizza's. The coke count was right and the pizza went to x16.
//
// A bare count belongs to the item it is spoken next to. "two cokes" scopes to
// the coke and to nothing else. Contextual forms ("make it 2", "another") carry
// no item of their own and legitimately refer to whatever line is being written,
// so they are returned unchanged.
//
// Deliberately conservative: when we cannot tell, we return null. Dropping a
// count means the customer is asked again. Applying it to the wrong line means
// we charge for sixteen pizzas.
export function parseExplicitQuantityForItem(
  text: string,
  ...
  // Contextual forms refer to the line under discussion, not to a named item.
  if (/\bmake (?:it|that)\s+(\d+|one|two|three|four|five|six|seven|eight)\b/.test(t)) {
    return parseExplicitQuantity(text);
  }
  if (/\banother\b|\bone more\b|\ban extra\b/.test(t)) {
    return parseExplicitQuantity(text);
  }

  const parsed = parseExplicitQuantity(text);
  if (!parsed) return null;
  if (!itemName) return null; // cannot scope -> do not apply

  // Significant tokens of the item's own name, singular-normalised.
  const stop = new Set(["the","a","an","of","with","and","or","large","small","medium","regular"]);
  const itemTokens = itemName.toLowerCase()
    .replace(/[^a-z0-9\s]/g, " ")
    .split(/\s+/)
  ...
  // The count must be spoken immediately before a mention of this item:
  // <number> [up to 3 filler words] <item token>
  const numWord = "(?:\\d+|two|three|four|five|six|seven|eight)";
  for (const tok of itemTokens) {
    const re = new RegExp(`\\b${numWord}\\b(?:\\s+\\w+){0,3}?\\s+\\w*${tok}\\w*`, "i");
    if (re.test(t)) return parsed;
  }
  return null;
}
