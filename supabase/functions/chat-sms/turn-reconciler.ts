...
export interface WriteCartLineInput {
  ...
  // "another one") — parseExplicitQuantity's output, computed by the
  // caller. The only thing that can grow an existing line's quantity;
  // omitted/null means no signal, so an existing line's quantity never
  // changes (idempotent no-op — CartOps' "never additive by default" rule).
...
export function writeCartLine(cart: ReconcilerCartLine[], input: WriteCartLineInput): WriteCartLineResult {
  if (input.continuationIndex !== undefined && input.continuationIndex >= 0) {
    ...
    else if (input.explicitQuantity?.kind === "relative") finalQty = preQty + input.explicitQuantity.delta;
