...
| L | PROVE demo kit (3 codes) | in verification | Measured 2026-09-06 without a phone: each code driven through its **real generator**, decoded with an independent decoder (OpenCV), after a known-good `api.qrserver.com` QR was decoded first as a control. Report: `docs/specs/2026-09-06-item-L-demo-kit.md`. **The kit Erin's email links to (`/admin/demo-kit`) is sound** — `qrcode.react` 4.2.0, all three codes decode and match their payloads byte-for-byte. All three destinations return 200 live. NJB kit payloads correct. | **L-1 CLOSED 2026-09-06 08:5x EDT** (f90e967): `vitos-demo.html` removed from the build allowlist and deleted — it was unlinked (Erin's email points to the vetted `/admin/demo-kit`), shipped an unfixable hand-rolled QR encoder, and was the same duplicate-artifact drift that caused a dead demo number twice. Verified live: `getsprintai.com/vitos-demo.html` → **404**, site otherwise intact. Single kit source of truth is now `/admin/demo-kit`. **L-2 MITIGATED 2026-09-08 20:22 EDT (a2d7631), no longer demo-blocking.** The email now leads with Vito's number printed in plain text — `(610) 735-8315`, real Telnyx carrier line, 10DLC-approved 2026-09-08 — plus a `sms:` tap-to-text link, both requiring no login. **Verified 2026-09-09** against the live shop record: `shops.phone_number_e164` for `vitos-pizza` = `+16107358315`, matches the email byte-for-byte. Erin can run the core "order as a customer" leg of the demo standing in a shop with zero authentication; the email's copy says exactly that ("If you can't get in, run the demo off the number above"). The gated `/admin/demo-kit` page is now positioned as a bonus (QR-scan convenience + the owner-dashboard walkthrough), not the critical path. **Downgraded, not closed:** the original question — does Erin get a real `shop_owner` login, or should `/admin/demo-kit` become a public no-login page — is still open, but it's a nice-to-have for the dashboard leg of the walkthrough, not a blocker for the sale. No further vigil deadline warranted; revisit only if Jason wants the fuller kit experience login-free. Physical phone scan + human walk remain Jason's leg. Unconfirmed: whether `erin-vitos-demo-email.html` has actually been sent to Erin yet — that send is Jason's call, not mine to trigger. |
