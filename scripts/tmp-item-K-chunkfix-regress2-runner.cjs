 ...
 * RE-RUN against the DEPLOYED scrape-shop (the original chunkfix-regress
 * run drove a LOCAL deno instance that lacked FIRECRAWL_API_KEY; those
 * results are void). Same 3 sites, same order, as the original run.
 * Fresh throwaway tenant. Writes ids/results to -regress2- files.
 */
const fs = require("fs");
...
(async () => {
  ...
  const idsPath = __dirname + "/tmp-item-K-chunkfix-regress2-ids.json";
  ...
  const TAG = "itemk-chunkfix-regress2-20260908";
  const tenant = await rest("POST", "/tenants", {
    name: "ITEMK Chunkfix Regress2 (delete me)", slug: `${TAG}-tenant`, status: "onboarding",
  ...
  for (let i = 0; i < SITES.length; i++) {
    ...
    const shop = await rest("POST", "/shops", {
      tenant_id, name: `ITEMK-CFR2 site${n}`, slug: `${TAG}-shop-${n}`,
    ...
    const r = {
      ...
      menu_extraction_chunks_failed: inv.body?.menu_extraction_chunks_failed ?? null,
      menu_extraction_truncated: inv.body?.menu_extraction_truncated ?? null,
    ...
    results.push(r);
    console.log(`[site ${n}] http=${r.http} ms=${r.ms} fn_ok=${r.fn_ok} status=${r.crawl_status} items_db=${r.items_in_db} priced=${r.items_with_price} chunks_failed=${r.menu_extraction_chunks_failed} truncated=${r.menu_extraction_truncated}`);
    if (r.fn_error) console.log(`   fn_error: ${JSON.stringify(r.fn_error).slice(0,300)}`);
    fs.writeFileSync(__dirname + "/tmp-item-K-chunkfix-regress2-results.json", JSON.stringify(results, null, 2));
