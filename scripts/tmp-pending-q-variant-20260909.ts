...
const TURNS = Deno.args.slice(1);

const sessionId = `pendingq-variant-run${RUN}-${Date.now()}-${Math.floor(Math.random() * 1e6)}`;
console.log(`\n=== PENDING-QUESTION VARIANT ${RUN} (session ${sessionId}) ===`);
for (const turn of TURNS) {
  ...
  for (const c of cartArr) {
    console.log(`  - ${c.name} qty=${c.quantity} price_cents=${c.price_cents}`);
  }
}
