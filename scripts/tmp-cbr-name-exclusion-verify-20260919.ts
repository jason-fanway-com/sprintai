// PO dispatch 2026-09-19: a7266e41 REJECTED (Chicken Bacon Ranch pizza
// double-charged "Bacon" as a standalone topping because it's part of the
// item's own name). Fix: recoverAssertedChoiceFromText / -ChoicesFromText
// now exclude any candidate whose full word set is already contained in
// the resolved item's own name. This is the mandatory 9-case verification
// matrix from the PO, run end-to-end (real PROPOSE model call -> decide()),
// same harness pattern as tmp-pepperoni-round3-repro-20260919.ts.
...
async function runReal(label: string, message: string) {
  ...
  const proposal: Proposal = proposeResult.proposal;
  console.log(`  raw proposal.adds = ${JSON.stringify(proposal.adds)}`);
  ...
  console.log(`  unresolvedSpans: ${JSON.stringify(result.unresolvedSpans)}`);
  console.log(`  disambiguationCandidateIds: ${JSON.stringify(result.disambiguationCandidateIds)}`);
  console.log("");
}

await runReal("case1", "a large cheese pizza with half pepperoni");
await runReal("case2", "a pepperoni stromboli");
await runReal("case3", "a Medium Hawaiian pizza with half sausage and half mushrooms");
await runReal("case4", "2 Medium Chicken Bacon Ranch pizzas");
await runReal("case5", "a House pizza, 16 inches");
await runReal("case6", "2 large pepperoni pizzas");
await runReal("case7", "house salad w/ black diamond steak");
await runReal("case8", "a chicken cheesesteak sandwich and a house salad");
await runReal("case9", "chicken bacon ranch flatbread");
