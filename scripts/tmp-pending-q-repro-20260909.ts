...
const TURNS = [
  "I need to order four large pizzas",
  "all plain",
];

const sessionId = `pendingq-repro-run${RUN}-${Date.now()}-${Math.floor(Math.random() * 1e6)}`;
console.log(`\n=== PENDING-QUESTION REPRO ${RUN} (session ${sessionId}) ===`);
