// Probe for M1 (2026-09-19, live conv d95306c8 #26): two pizzas named in one
// message, each with its own size — "Sausage" wrongly landing as a topping
// on the Gourmet White Fiesta AND as its own pending pizza (double-charge),
// plus sizes crossing between the two items. Same harness pattern as
// tmp-pepperoni-warts-probe-20260919.ts: real propose.ts model call + real
// decide() against live Vito's data.
...
console.log("=== sausage/gourmet white fiesta menu items ===");
for (const i of items) if (/sausage|gourmet white fiesta/i.test(i.name)) console.log(`  ${i.name} | id=${i.id} | category=${i.category} | $${(i.price_cents/100).toFixed(2)}`);
...
async function reportReal(label: string, message: string) {
  ...
  console.log(`  disambiguationSpanText: ${JSON.stringify(result.disambiguationSpanText)}`);
  console.log("");
}

console.log("=== M1: two pizzas, one message, sizes crossed + sausage double-charged ===");
await reportReal("m1", "Hi, I'd like to order a Sausage Pizza - Small and a Gourmet White Fiesta - Large, please.");
