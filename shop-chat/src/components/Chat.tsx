...
import { Send, RefreshCw, MessageSquare, Sparkles, Mic, MicOff, Undo2, X, Check, LogOut, Store } from 'lucide-react'
...
export default function Chat({ shopId, onLogout, onSwitchShop, shopName }: ChatProps) {
  ...
  const undoAction = async (_undoToken: string) => {
    setIsLoading(true)
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session?.access_token) {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      {/* Live Status Header */}
      {statusHeader && (
        <div className="safe-area-top px-3 py-2 bg-gray-50 border-b border-gray-200 flex items-center gap-2 text-xs flex-wrap">
          ...
          return (
            <div key={i} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
              <div className="max-w-[85%] space-y-2">
                ...
                {/* Clarification Card (LLM needs disambiguation — tappable options) */}
                {hasConfirmationCard && (msg.confirmationCard!.details as Record<string, unknown>).needs_clarification === true && (
                  <div className="bg-white border-2 border-amber-300 rounded-xl p-3 shadow-sm">
                    <p className="text-sm text-gray-700 mb-3 font-medium">{msg.confirmationCard!.summary}</p>
                    <div className="flex flex-wrap gap-2">
                      {((msg.confirmationCard!.details as Record<string, unknown>).options as string[] ?? []).map((option, oi) => (
                        <button
                          key={oi}
                          onClick={() => {
                            const chosen = option
                            setMessages(prev => prev.map((m, idx) => {
                              if (idx === i) {
                                ...
                                return { ...rest, content: m.confirmationCard!.summary + ' → ' + chosen }
                              }
                              return m
                            }))
                            sendMessage(chosen)
                    ...
                    <button
                      onClick={() => {
                        setMessages(prev => prev.map((m, idx) => {
                          if (idx === i) {
                            const { confirmationCard, ...rest } = m
                            return { ...rest, content: m.confirmationCard!.summary + ' (dismissed)' }
                          }
                          return m
                        }))
                      }}
                ...
                {/* Confirmation Card (proposal ready — Confirm/Cancel) */}
                {hasConfirmationCard && (msg.confirmationCard!.details as Record<string, unknown>).needs_clarification !== true && (
      ...
      {/* Quick-action chips */}
      <div className="px-3 py-2 flex gap-2 overflow-x-auto border-t border-gray-100">
      ...
      {/* Input */}
      <div className="safe-area-bottom px-3 py-3 border-t border-gray-200">
        <form
